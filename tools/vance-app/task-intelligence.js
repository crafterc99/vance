/**
 * VANCE — Task Intelligence System
 *
 * Analyzes every conversation message and proactively:
 *   - Extracts actionable tasks from natural speech
 *   - Classifies: user-task (user does it) vs vance-task (Vance does it) vs note
 ...
 *
 * This makes Vance proactive — not waiting to be told "create a task",
...
// Patterns that signal "do this FOR me" (Vance task)
const VANCE_TASK_PATTERNS = [
...
const PROJECT_KEYWORDS = {
  ...
  'vance': ['vance', 'yourself', 'your own', 'your code', 'the assistant', 'command center'],
};
...
class TaskIntelligence {
  ...
  analyzeMessage(message, projectContext = null) {
    ...
    // Check if this is a Vance task (user wants Vance to do something)
    const isVanceTask = VANCE_TASK_PATTERNS.some(p => p.test(message));
  ...
  addPriority(title, opts = {}) {
    const priority = {
      ...
      linkedTasks: [],   // task IDs (both user and vance tasks)
      createdAt: new Date().toISOString(),
    };
  ...
  /**
   * Auto-queue a Vance coding task from conversation analysis.
   ...
   */
  getDashboard() {
    const vanceTasks = this.taskManager ? {
    ...
    return {
      ...
      stats: {
        ...
        totalQueuedTasks: vanceTasks.queued.length,
        isRunning: !!vanceTasks.running,
