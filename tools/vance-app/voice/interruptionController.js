/**
 * VANCE — Interruption Controller
 *
 * Manages the interruption system for voice conversations.
 * When the user starts speaking while Vance is talking:
 ...
 *
 * This gives a natural conversational feel where the user can cut Vance off.
 */
...
class InterruptionController extends EventEmitter {
  ...
  /**
   * Called when VAD detects speech while Vance might be talking
   */
  _onSpeechStart() {
    if (!this.enabled) return;
    if (!this.tts.isSpeaking()) return; // Vance isn't speaking, no interruption needed
