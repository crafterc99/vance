(function () {
  ...
  // ─── Simplex 3D noise (Ashima, public domain) embedded as GLSL ───
  ...
  function updateOrb(time) {
    ...
    // ── Glass shell ──
    if (orbGlassShell) {
      orbGlassShell.material.uniforms.uTime.value = time;
  ...
  function getProjectMeshColor(p) {
    ...
    if (s.includes('blair')) return COLORS.accent;
  ...
  function handleWSMessage(msg) {
    switch (msg.type) {
      ...
      case 'project-created':
        ...
        break;
      case 'voice-state': {
        const vs = msg.state || 'idle';
        const intensityMap = { idle: 0.1, listening: 0.2, thinking: 0.5, speaking: 1.0 };
        voiceTargetIntensity = intensityMap[vs] !== undefined ? intensityMap[vs] : 0.1;
        break;
      }
