/**
 * BLAIR Command Center — Main Controller
...
(function () {
    ...
    switch (d.type) {
      ...
      case 'task-intelligence':
        if (d.action === 'user-task-detected') {
          ...
          renderTaskDashboard();
        } else if (d.action === 'blair-task-queued') {
  ...
  function showWelcome() {
    ...
    el.innerHTML = `
      <div class="welcome" id="welcome">
        ...
        <div class="welcome-logo">BLAIR</div>
  ...
  function startBusyWatchdog() {
    ...
    state.busyTimer = setInterval(() => {
      ...
      if (Date.now() - state.lastActivity > 60000) {
        console.warn('[Blair] Busy watchdog triggered — force unlocking input after 60s inactivity');
        state.isBusy = false;
        clearBusyWatchdog();
        setStatus('online', 'Online');
        removeThinking();
  ...
  function renderTaskDashboard() {
    ...
    // ─── Running Task ───
    const running = tasks.find(t => t.status === 'running') || dash?.blairTasks?.running;
    if (running) {
