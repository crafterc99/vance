/**
 * BLAIR — Unified Coding Executor (Agent SDK)
 *
...
function prepareGitBranch(task) {
  ...
  const blairRoot = path.resolve(__dirname, '../..');
  if (path.resolve(cwd) === blairDir || path.resolve(cwd) === blairRoot) {
    return null;
  }
  ...
  const branch = `blair/${slug}`;
  let stashed = false;

  try {
    const status = execSync('git status --porcelain', { cwd, encoding: 'utf8' }).trim();
    if (status) {
      execSync('git stash push -m "blair-auto-stash"', { cwd, stdio: 'pipe' });
      stashed = true;
    }
...
function postTaskGit(task) {
  ...
  try {
    ...
    if (status) {
      ...
      execSync(`git commit -m "blair: final changes for ${task.title}"`, { cwd, stdio: 'pipe' });
    }
...
function runInteractive(opts) {
  const systemAppend = [
    'You are operating as Claude Code, controlled by Blair (a JARVIS-like AI).',
